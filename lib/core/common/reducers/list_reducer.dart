...
Map<String, double> getTotals(
  ...
  List<BillingModel> products = entities
      ...
      .map((e) => BillingModel(
            ...
            rate: stock[e.key].stock.masterRate,
          ))
      .toList();
  ...
  for (var item in _taxExclusive) {
    ...
    final costAfter = (calc + item.rate) * item.quantity;
    taxExc.add({'tax': calc, 'costAfter': costAfter});
  }
  ...
  for (var item in _taxInclusive) {
    ...
    final rate = item.rate;
    final calc = (taxPct / 100) * item.rate * item.quantity;
    final costAfter = (item.rate - calc) * item.quantity;
    taxInc.add({'tax': calc, 'costAfter': costAfter});
  }
  ...
  return {'total': total, 'tax': totalTax, 'subtotal': subtotal};
}
