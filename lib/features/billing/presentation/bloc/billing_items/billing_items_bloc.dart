    ...
    final result = await getIt<BillingItemsLocalDataSource>()
        .checkoutBillingItems(
            TransactionTableCompanion(
              ...
              locationCode: Value(event.transaction.locationCode),
              storecode: Value(event.transaction.storecode),
              payComp: Value(event.transaction.payComp),
            ),
            event.invoices
                .map((e) => InvoiceTableCompanion(
                    ...
                    taxPct: Value(e.taxPct),
                    availQty: Value(e.availQty)))
                .toList())
